...
//        $products = Product::get(); //产品列表
    ...
    public function setBrokeragePost()
    {
        $input = $this->request->all();
        if(count($input)=='0'){
            return back()->withErrors('请检查产品是否已录入佣金');
        }
        ...
        foreach($input['brokerage'] as $k => $v){
            ...
            foreach($p_brokerage as $p_k => $p_v){
                if(!is_numeric($v))
                    return redirect(url()->previous())->withErrors('请输入有效的佣金比例值');
                ...
                if(((int)$input['by_stages_way'][$k] == $p_v['by_stages_way']) && ($v < 0))
                    return redirect(url()->previous())->withErrors('渠道/代理人比例 不得小于0');
            }
