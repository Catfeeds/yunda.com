...
class PolicyController extends BaseController
{
    ...
    public function index()
    {
        ...
        $status_id = $input['status_id'] ?? "";//保单状态:全部保单-1，保障中1，失效2，待生效0，退保3
        $date = $input['date'] ?? "";//订单日期
        $date_start = $input['date_start'] ?? "";//订单开始时间
        $date_end =$input['date_end'] ?? "";//订单结束时间
        $deal_type =$input['type'] ?? "";//成交方式：线上0，线下1，全部-2
        $agent =$input['agent'] ?? "";//代理人 全部-2
        ...
        $start = $start??"";
        $end = $end??"";
        return view('backend_v2.policy.index',compact("count","list","status_id","deal_type","data_agent","agent","start","end"));
    }
    //个人保单详情
