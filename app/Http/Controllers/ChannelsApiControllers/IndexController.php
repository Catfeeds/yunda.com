...
class IndexController extends BaseController{
    ...
    //保险错误提示页URL
    const INSURE_ERROR_URL = '/api/channels/yunda/ins_error';
    //我的保险页URL
    const DO_INSURE_URL = '/channelsapi/do_insure';
    //保险购买页URL
    const TO_INSURE_URL = '/channelsapi/to_insure';
    //初始化
                ...
                if($status=='200'){
                    ...
                    if(is_null($channel_res)){
                        ...
                        Channel::insert([
                            ...
                            'url'=>$channel_info['call_back_url'],
                            'code'=>$channel_info['code'],
                            'only_id'=>$channel_info['account_id'],
                            'sign_key'=>$channel_info['sign_key'],
...
//                return json_encode(['status'=>'200','content'=>['account_id'=>$channel_email_res->only_id,'sign_key'=>$channel_email_res->sign_key,'account'=>$channel_email_res->name]],JSON_UNESCAPED_UNICODE);
    ...
    public function getParams()
    {
        ...
        switch ($biz_content['channel_code']){
            case '91110116MA00CAPM96':
                return $this->saveDouwanInfo($biz_content);
                break;
            case '913101070558512086':
            ...
            case 'YD':
                    ...
                    $insure_start = time();//投保时间
                    $channel_bank_code = $biz_content['channel_bank_code'];//银行卡号
                    $channel_bank_name = $biz_content['channel_bank_name'];//银行名称
                    $channel_provinces = $biz_content['channel_provinces'];//省
                    $channel_city = $biz_content['channel_city'];//市
                    $channel_county = $biz_content['channel_county'];//县
                    //姓名，身份证信息，手机号判空
                    if(!$channel_user_name||!$channel_user_code||!$channel_user_phone){
                        return json_encode(['status'=>'500','msg'=>'投保失败，请绑定银行卡','url'=>'http://'.$_SERVER['HTTP_HOST'].self::INSURE_ERROR_URL.'/empty'],JSON_UNESCAPED_UNICODE);
                    }
                    //todo 联合登录信息里没有银行卡信息，直接返回投保失败，引导前往绑定银行卡页面
                    if(!$channel_bank_code){
                        return json_encode(['status'=>'500','msg'=>'投保失败，请绑定银行卡','url'=>'http://'.$_SERVER['HTTP_HOST'].self::INSURE_ERROR_URL.'/no_bank'],JSON_UNESCAPED_UNICODE);
                    }
    ...
    public function doInsure(){
        ...
        return view('channels.yunda.do_insure')
